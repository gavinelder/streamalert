...
class CLIConfig:
    ...
    def _setup_build_directory(self, directory):
        ...
        if os.path.exists(directory):
            shutil.rmtree(
                directory
            )  # shutil.copytree in python3.7 cannot handle existing dir
        ...
        for function in lambda_functions:
            ...
            if function not in CLUSTERED_FUNCTIONS:
                ...
                for cluster in clusters:
                    self.config["clusters"][cluster][function_config][
                        "enable_custom_metrics"
                    ] = enabled
    ...
    def _add_metric_alarm_config(alarm_info, current_alarms):
        ...
        current_alarms[alarm_info["alarm_name"]] = {
            key: value for key, value in alarm_info.items() if key not in omitted_keys
        }
    ...
    def _alarm_exists(self, alarm_name):
        ...
        """
        message = (
            "CloudWatch metric alarm names must be unique "
    ...
    def _clusters_with_metrics_enabled(self, function):
        ...
        return {
            ...
            for cluster, cluster_config in self.config["clusters"].items()
            if (
                self.config["clusters"][cluster][function_config].get(
                    "enable_custom_metrics"
                )
            )
        }
    ...
    def _add_global_metric_alarm(self, alarm_info):
        ...
        alarm_settings = alarm_info.copy()
        alarm_settings["metric_name"] = "{}-{}".format(
            metrics.FUNC_PREFIXES[function_name], alarm_settings["metric_name"]
        )
        ...
        LOGGER.info(
            "Successfully added '%s' metric alarm to " "'conf/lambda.json'.",
            alarm_settings["alarm_name"],
        )
    ...
    def add_app(self, func_name, app_info):
        ...
        if prompt_for_auth and not save_app_auth_info(
            app, app_info, func_name, overwrite
        ):
            return False
        ...
        for config_key in self.config:
            ...
            if config_key == "clusters":
                ...
                for cluster_key in self.config["clusters"]:
                    ...
                    self._config_writer(
                        format_path(parts), self.config["clusters"][cluster_key]
                    )
