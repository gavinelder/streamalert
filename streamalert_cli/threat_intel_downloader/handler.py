...
class ThreatIntelDownloaderCommand(CLICommand):
    ...
    def setup_subparser(cls, subparser):
        ...
        ti_subparsers = subparser.add_subparsers(
            dest="threat-intel-downloader subcommand", required=True
        )
    ...
    def handler(cls, options, config):
        ...
        if options.subcommand == "enable":
            ...
            return save_api_creds_info(
                config["global"]["account"]["region"], overwrite=True
            )
...
def save_api_creds_info(region, overwrite=False):
    ...
    required_creds = {
        "api_user": {
            "description": (
                "API username to retrieve IOCs via API calls. "
    ...
    saved = save_parameter(
        region, ThreatStream.CRED_PARAMETER_NAME, creds_dict, description, overwrite
    )
    if saved:
        LOGGER.info(
        ...
        LOGGER.error(
            "Threat Intel Downloader credentials were not saved to " "parameter store."
        )
