...
def generate_cloudtrail(cluster_name, cluster_dict, config):
    ...
    s3_bucket_name = s3_settings.get(
        "bucket_name", "{}-{}-streamalert-cloudtrail".format(prefix, cluster_name)
    )
    ...
    logging_bucket, _ = s3_access_logging_bucket(
        config
    )  # Just get the bucket name from the tuple
    ...
    if send_to_cloudwatch:
        ...
        ] = "${{module.cloudtrail_cloudwatch_{}.cloudwatch_logs_group_arn}}".format(
            cluster_name
        )
            ...
            s3_bucket_name: [
                {
                    "filter_prefix": (
                        ...
                        if ignore_digest
                        else "AWSLogs/{}/".format(account_id)
                    )
                }
...
def generate_cloudtrail_cloudwatch(
    cluster_name, cluster_dict, config, settings, prefix, region
):
    ...
    if not destination_arn:
        ...
        if not generate_cloudwatch_destinations_internal(
    ...
    cluster_dict["module"][
        "cloudtrail_cloudwatch_{}".format(cluster_name)
    ] = module_info
