...
def write_vars(config, **kwargs):
    ...
    _create_terraform_module_file(
        kwargs, os.path.join(config.build_directory, "terraform.tfvars.json")
    )
...
def generate_s3_bucket(bucket, logging, **kwargs):
    ...
    s3_bucket = {
        ...
        "logging": {"target_bucket": logging, "target_prefix": "{}/".format(bucket)},
...
def generate_main(config, init=False):
    ...
    main_dict["resource"]["aws_s3_bucket"] = {
        "streamalerts": generate_s3_bucket(
            bucket=firehose_alerts_bucket(config), logging=logging_bucket
        )
    }
    ...
    if create_state_bucket:
        ...
        ] = generate_s3_bucket(bucket=terraform_bucket_name, logging=logging_bucket)
        ...
        "policy": json.dumps(
            {
                ...
                "Statement": [
                    ...
                    },
                    {
                        ...
                        "Condition": {
                            "StringEquals": {
                                "kms:CallerAccount": config["global"]["account"][
                                    "aws_account_id"
                                ]
                            }
                        },
...
def terraform_generate_handler(config, init=False, check_tf=True, check_creds=True):
    ...
    if metric_alarms:
        _create_terraform_module_file(
            metric_alarms, os.path.join(config.build_directory, "metric_alarms.tf.json")
        )
...
def _generate_artifact_extractor_module(config):
    ...
    if "artifact_extractor" in config["global"]["infrastructure"]:
        if config["global"]["infrastructure"]["artifact_extractor"].get("enabled"):
            _create_terraform_module_file(
                generate_artifact_extractor(config), tf_file_name
            )
            return
...
def generate_global_lambda_settings(
    config, conf_name, generate_func, tf_tmp_file_name, required=True
):
    ...
    tf_tmp_file = os.path.join(
        config.build_directory, "{}.tf.json".format(tf_tmp_file_name)
    )
    ...
    if not config["lambda"].get(conf_name):
        LOGGER.warning(
            "Optional configuration missing in lambda.json, skipping: %s", conf_name
        )
