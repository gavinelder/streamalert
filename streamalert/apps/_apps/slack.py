...
class SlackApp(AppIntegration):
    ...
    def _sleep_seconds(cls):
        raise NotImplementedError(
            "Subclasses should implement the _sleep_seconds method"
        )
    ...
    def _required_auth_info(cls):
        ...
        return {
            "auth_token": {
                "description": (
                    "The security token generated by installing an app. "
    ...
    def _check_for_more_to_poll(self, response):
        self._next_page += 1
        return not (
            "paging" in list(response.keys())
            and response["paging"]["pages"] == response["paging"]["page"]
        )
    ...
    def _filter_response_entries(self, response):
        ...
        raise NotImplementedError(
            "Subclasses must implement the _filter_response_entries method"
        )
    ...
    def _gather_logs(self):
        ...
        if not response.get("ok"):
            LOGGER.error(
                "Received error or warning from slack: %s", response.get("error")
            )
            return False
...
class SlackAccessApp(SlackApp):
    ...
    def _filter_response_entries(self, response):
        if "logins" in response:
            return [
                x for x in response["logins"] if x["date_last"] > self._last_timestamp
            ]
        return False
    ...
    def _check_for_more_to_poll(self, response):
        ...
        if (
            response["paging"]["page"] >= self._SLACK_API_MAX_PAGE_COUNT
...
class SlackIntegrationsApp(SlackApp):
    ...
    def _filter_response_entries(self, response):
        if "logs" in response:
            return [
                x for x in response["logs"] if int(x["date"]) > self._last_timestamp
            ]
        return False
