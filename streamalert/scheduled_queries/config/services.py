...
def configure_container(container):
    ...
    container.register(
        ServiceDefinition("query_parameter_generator", _make_param_generator)
    )
    container.register(
        ServiceDefinition("query_pack_repository", _make_query_pack_repo)
    )
    container.register(
        ServiceDefinition("query_pack_manager_factory", _make_query_pack_factory)
    )
    container.register(
        ServiceDefinition("query_pack_execution_context", _make_execution_context)
    )
    container.register(ServiceDefinition("clock", _make_clock))
    container.register(
        ServiceDefinition("boto3_athena_client", _make_boto3_athena_client)
    )
    container.register(
        ServiceDefinition("boto3_kinesis_client", _make_boto3_kinesis_client)
    )
...
def _make_cache(container):
    cache = StateManager(logger=container.get("logger"))
...
def _make_boto3_athena_client(container):
    ...
    config = botocore_client.Config(
        connect_timeout=5, read_timeout=5, region_name=region
    )
