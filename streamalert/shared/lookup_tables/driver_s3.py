...
class S3Driver(PersistenceDriver):
    ...
    def __init__(self, configuration):
        ...
        self._cache_refresh_minutes = configuration.get(
            "cache_refresh_minutes", self._DEFAULT_CACHE_REFRESH_MINUTES
        )
    ...
    def commit(self):
        if not self._dirty:
            LOGGER.warning(
                "LookupTable (%s): Empty commit; no records dirtied", self.id
            )
            return
    ...
    def _reload_if_necessary(self):
        ...
        """
        if self._cache_clock.has(self._MAGIC_CACHE_TTL_KEY) and self._cache_clock.get(
            self._MAGIC_CACHE_TTL_KEY, False
        ):
            LOGGER.debug(
    ...
    def _reload(self):
        ...
        self._cache_clock.set(
            self._MAGIC_CACHE_TTL_KEY, True, self._cache_refresh_minutes
        )
    ...
    def upload(self, bytes_data):
        ...
        try:
            self._s3_client.Bucket(self._s3_bucket).put_object(
                Key=self._s3_key, Body=bytes_data
            )
            LOGGER.debug(
                "LookupTable (%s): Object successfully uploaded to S3", self._driver.id
            )
