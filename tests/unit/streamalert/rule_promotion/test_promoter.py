...
class TestRulePromoter:
    ...
    def setup(self):
        ...
        with patch(
            "streamalert.rule_promotion.promoter.load_config"
        ) as config_mock, patch(
            "streamalert.rule_promotion.promoter.StatsPublisher", Mock()
        ), patch(
            "boto3.client", _mock_boto
        ), patch.dict(
    ...
    def _add_fake_stats(self):
        ...
        self.promoter._staging_stats.update(
            {
                "test_rule": StagingStatistic(
                    time, time + timedelta(days=2), time, "test_rule"
                ),
                "test_rule_2": StagingStatistic(
                    time, time + timedelta(days=2), time, "test_rule_2"
                ),
            }
        )
    ...
    @patch(
        "streamalert.shared.rule_table.RuleTable.remote_rule_info",
    ...
    def test_run(self, publish_mock, table_mock):
        ...
        with patch.object(self.promoter, "_update_alert_count", Mock()), patch.object(
            self.promoter, "_promote_rules", Mock()
        ):
    ...
    def test_promote_rules(self, log_mock):
        ...
        self.promoter._current_time = self.promoter._staging_stats[
            "test_rule"
        ].staged_until + timedelta(hours=1)
