...
from pytest import assert_equal, assert_false, assert_count_equal, raises
import requests
...
class TestDuoApp:
    ...
    def test_required_auth_info(self):
        ...
        ) == collections.Counter({"api_hostname", "integration_key", "secret_key"})
    ...
    def test_gather_logs(self, requests_mock):
        ...
        requests_mock.return_value = Mock(
            status_code=200, json=Mock(return_value={"response": logs})
        )
    ...
    def test_gather_logs_empty(self, requests_mock):
        ...
        requests_mock.return_value = Mock(
            status_code=200, json=Mock(side_effect=[{"response": []}])
        )
