...
class TestAppIntegration:
    ...
    @patch("time.sleep")
    @patch(
        "streamalert.apps.app_base.AppIntegration._sleep_seconds", Mock(return_value=1)
    )
    ...
    def test_report_time(self, time_mock, log_mock):
        ...
        log_mock.assert_called_with(
            "[%s] Function executed in %.4f seconds.", "_test", 200.0
        )
    ...
    def test_required_auth_info(self, auth_mock):
        ...
        assert self._app.required_auth_info() == dict()
    ...
    def test_check_http_response_bad(self, log_mock):
        ...
        log_mock.assert_called_with(
            "[%s] HTTP request failed: [%d] %s", self._app, 404, "hey"
        )
    ...
    def test_initialize_partial(self, log_mock):
        ...
        assert not self._app._initialize()
        log_mock.assert_called_with(
            "[%s] App in partial execution state, exiting", self._app
        )
    ...
    def test_invoke_successive_app(self, log_mock, boto_mock):
        ...
        boto_mock.return_value.invoke.return_value = {
            "ResponseMetadata": {"RequestId": "foobar"}
        }
        self._app._invoke_successive_app()
    ...
    def test_make_post_request_json(self, requests_mock):
        ...
        requests_mock.return_value = Mock(
            status_code=200, json=Mock(return_value=message)
        )
        args = "hostname"
    ...
    def test_gather_no_logs(self, log_mock):
        ...
        with patch.object(AppIntegration, "_gather_logs") as subclass_gather_mock:
            ...
            log_mock.assert_called_with(
                "[%s] Gather process was not able to poll any logs on poll #%d",
                self._app,
                1,
            )
    ...
    @patch(
        "streamalert.apps.config.AppConfig.remaining_ms",
