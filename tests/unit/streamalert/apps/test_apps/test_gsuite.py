...
from pytest import assert_equal, assert_false, assert_count_equal, assert_true, raises
...
class TestGSuiteReportsApp:
    ...
    @patch(
        "streamalert.apps._apps.gsuite.GSuiteReportsApp._load_credentials",
    ...
    def test_create_service_ssl_error(self, build_mock, log_mock):
        ...
        build_mock.side_effect = ssl.SSLError(
            "_ssl.c:574: The handshake operation timed out"
        )
        assert not self._app._create_service()
        log_mock.assert_called_with("[%s] Failed to build discovery service", self._app)
    ...
    @patch(
        "streamalert.apps._apps.gsuite.GSuiteReportsApp._create_service",
        Mock(return_value=True),
    )
    @patch("logging.Logger.exception")
    def test_gather_logs_http_error(self, log_mock):
        ...
        with patch.object(self._app, "_activities_service") as service_mock:
            ...
            log_mock.assert_called_with(
                "[%s] Failed to execute activities listing", self._app
            )
    ...
    def _get_sample_logs(count):
        ...
        def _get_timestamp(start_timestamp, subtract_seconds):
            timestamp = datetime.strptime(
                start_timestamp, GSuiteReportsApp.date_formatter()
            )
