...
class TestConfigLoading(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock_cluster_contents = (
            '{"data_sources": {}, "classifier_config": {"foo": "bar"}}'
        )
        ...
        self.fs.create_file(
            "conf_schemas/clusters/prod.json", contents=mock_cluster_contents
        )
        self.fs.create_file(
            "conf_schemas/clusters/dev.json", contents=mock_cluster_contents
        )
    ...
    def test_load_include():
        ...
        assert collections.Counter(list(config.keys())) == collections.Counter(
            expected_keys
        )
        assert collections.Counter(
            list(config["clusters"].keys())
        ) == collections.Counter(expected_clusters_keys)
...
class TestConfigArtifactExtractor:
    ...
    def setup(self):
        self.default_conf_data = {
            "global": {
                "infrastructure": {
                    "firehose": {"enabled": False, "enabled_logs": {}},
                    "artifact_extractor": {"enabled": False},
                }
            },
            "logs": {
                "test_log:type_1": {
    ...
    def test_artifact_extractor(self):
        ...
        self.default_conf_data["global"]["infrastructure"]["artifact_extractor"][
            "enabled"
        ] = True
        assert not artifact_extractor_enabled(self.default_conf_data)
