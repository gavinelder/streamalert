...
class TestAlert:
    ...
    def _basic_alert():
        return Alert(
            "test_rule", {"abc": 123}, {"aws-firehose:alerts", "aws-sns:test-output"}
        )
    ...
    def test_init_invalid_kwargs(self):
        ...
        assert_raises(
            AlertCreationError, Alert, "", {}, set(), cluster="test", invalid="nonsense"
        )
    ...
    def test_can_merge_merge_keys_absent(self):
        ...
        alert1 = Alert(
            "", {}, set(), merge_by_keys=["key"], merge_window=timedelta(minutes=10)
        )
        alert2 = Alert(
            "", {}, set(), merge_by_keys=["key"], merge_window=timedelta(minutes=10)
        )
    ...
    def test_compute_common_partial_nested(self):
        ...
        assert {"abc": 123, "nested": {"A": 1}} == Alert._compute_common(
            [record1, record2]
        )
    ...
    def test_compute_diff_no_diff(self):
        ...
        record = {"a": 1, "b": 2, "c": {"d": {"e": 3}}}
        common = record
        assert {} == Alert._compute_diff(common, record)
    ...
    def test_merge(self):
        ...
        expected_record = {
            ...
            "MergedBy": {"hostname": "my-computer-name", "username": "user"},
            "OtherCommonKeys": {
                "alliance_data_virustotal": [],
                "alliance_link_virustotal": "",
                "alliance_score_virustotal": 0,
    ...
    def test_merge_nested(self):
        ...
        record1 = {
            "NumMatchedRules": 1,
            "FileInfo": {"Deleted": None, "Nested": [1, 2, "three"]},
            "MatchedRules": {"Rule1": "MatchedStrings"},
        }
        alert1 = Alert(
            "RuleName",
            record1,
        ...
        expected_record = {
            ...
            "MergedBy": {"Nested": [1, 2, "three"]},
            "OtherCommonKeys": {"MatchedRules": {"Rule1": "MatchedStrings"}},
            "ValueDiffs": {
                "2000-01-01T00:00:00.000000Z": {
                    "NumMatchedRules": 1,
                    "FileInfo": {"Deleted": None},
                },
                "2000-01-02T00:00:00.000000Z": {
                    "NumMatchedRules": 2,
                    "FileInfo": {"Deleted": None},
