...
class TestFirehoseClient:
    ...
    def setup(self):
        ...
        with patch("boto3.client"):  # patch to speed up unit tests slightly
            self._client = FirehoseClient(
                prefix="unit-test", firehose_config={"use_prefix": True}
            )
    ...
    def _sample_raw_records(cls, count=2):
        return [{"key_{}".format(i): "value_{}".format(i)} for i in range(count)]
    ...
    def test_load_from_config(self):
        ...
        with patch("boto3.client"):  # patch to speed up unit tests slightly
            client = FirehoseClient.load_from_config(
                prefix="unit-test", firehose_config={"enabled": True}, log_sources=None
            )
    ...
    def test_load_from_config_disabled(self):
        ...
        client = FirehoseClient.load_from_config(
            prefix="unit-test", firehose_config={}, log_sources=None
        )
        assert client == None
    ...
    def test_send(self, send_batch_mock):
        ...
        send_batch_mock.assert_called_with(
            "unit_test_streamalert_log_type_01_sub_type_01",
            expected_batch,
            "classifier",
        )
    ...
    def test_generate_firehose_name(self):
        ...
        results = [
            self._client.generate_firehose_name("", log_name) for log_name in log_names
        ]
    ...
    def test_artifacts_firehose_stream_name(self):
        ...
        config_data = {
            "global": {"account": {"prefix": "unittest"}},
            "lambda": {"artifact_extractor_config": {}},
        }
