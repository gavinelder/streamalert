...
from pytest import assert_equal, assert_not_equal, assert_raises
...
class TestRuleTable:
    ...
    def test_load_remote_state_state(self, window_mock):
        ...
        expected_state = {
            ...
            "now_staged_rule": {
                "Staged": True,
                "StagedAt": datetime(
                    year=2018, month=4, day=21, hour=2, minute=23, second=13
                ),
                "StagedUntil": datetime(
                    year=2018, month=4, day=23, hour=2, minute=23, second=13
                ),
            },
        }
    ...
    def test_toggle_staged_state_update(self, log_mock):
        ...
        rule_name = "staged_rule"
        staged = True
        self.rule_table._table.put_item(
            Item={
                "RuleName": rule_name,
                "Staged": staged,
    ...
    def test_toggle_staged_state_error(self):
        ...
        self._create_db_rule_with_name(rule_name, True)
        with patch.object(
            self.rule_table._table,
            "update_item",
            side_effect=ClientError({"Error": {"Code": "TEST"}}, "UpdateItem"),
        ):
            assert_raises(
                ClientError, self.rule_table.toggle_staged_state, rule_name, True
            )
