...
class TestAthenaClient:
    ...
    def test_init_fix_bucket_path(self, date_mock):
        ...
        with patch.dict(os.environ, {"AWS_DEFAULT_REGION": "us-west-1"}):
            client = AthenaClient(
                self._db_name, "test-streamalert-athena-results", "unit-test"
            )
    ...
    def test_get_table_partitions_error(self):
        ...
        assert_raises(
            AthenaQueryExecutionError, self.client.get_table_partitions, "test_table"
        )
    ...
    def test_query_result_paginator(self):
        ...
        assert collections.Counter(items) == collections.Counter(
            [{"ResultSet": {"Rows": [data]}}] * 4
        )
    ...
    def test_run_async_query_failure(self):
        ...
        self.client._client.raise_exception = True
        assert_raises(
            AthenaQueryExecutionError, self.client.run_async_query, "test query"
        )
