...
from pytest import assert_equal, assert_false
...
def generate_categorized_records(normalized=False, count=2):
    ...
    json_data = [{"key_{}".format(cnt): "value_{}".format(cnt)} for cnt in range(count)]

    if normalized:
        for data in json_data:
            data[Normalizer.NORMALIZATION_KEY] = {
                "normalized_type1": [{"values": ["value1"], "function": None}],
                "normalized_type2": [
                    {
