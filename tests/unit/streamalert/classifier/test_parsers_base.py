...
class TestParserBaseClassMethods:
    ...
    def test_apply_envelope(self):
        ...
        expected_record = {'key': 'value', 'streamalert:envelope_keys': {'env_key_01': 'value'}}
    ...
    def test_matches_log_patterns_invalid_key(self, log_mock):
        ...
        log_mock.assert_any_call('Declared log pattern key [%s] does exist in record:\n%s',
                                 'not_key', record)
    ...
    def test_key_check_debug(self, log_mock):
        ...
        with patch.object(parsers, 'LOGGER_DEBUG_ENABLED', True):
            assert ParserBase._key_check(record, schema) == False
            log_mock.assert_called_with('Nested key check failure. Schema:\n%s\nRecord:\n%s',
    ...
    def test_convert_type_unsupported_type(self, log_mock):
        ...
        log_mock.assert_called_with('Unsupported value type in schema for key \'%s\': %s', 'key',
                                    'foobar')
    ...
    def test_convert_type_optionals(self, log_mock):
        ...
        log_mock.assert_called_with('Skipping optional key not found in record: %s', 'optional_key')
