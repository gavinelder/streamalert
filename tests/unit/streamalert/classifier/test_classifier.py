...
class TestClassifier:
    ...
    def setup(self):
        """Classifier - Setup"""
        with patch.object(classifier_module, "Normalizer"), patch.object(
            classifier_module, "FirehoseClient"
        ), patch.object(classifier_module, "SQSClient"), patch.dict(
            os.environ, {"CLUSTER": "prod"}
        ), patch(
            "streamalert.classifier.classifier.config.load_config",
    ...
    def _mock_global(cls):
        return {
            "account": {"prefix": "unit-test"},
            "infrastructure": {
                "firehose": {"enabled": True, "enabled_logs": {"log_type_01": None}}
            },
        }
    ...
    def test_load_logs_for_resource(self):
        ...
        result = self._classifier._load_logs_for_resource(
    ...
    def test_classify_payload(self, process_mock):
        ...
        with patch.object(
            classifier_module, "Normalizer"
        ) as normalizer_mock, patch.object(Classifier, "_log_bad_records") as log_mock:
            ...
            process_mock.assert_called_with(
                ...
                OrderedDict(
                    [
                        (
                            "log_type_01:sub_type",
                            self._mock_logs()["log_type_01:sub_type"],
                        )
                    ]
                ),
            )
    ...
    def test_classify_payload_no_logs(self, log_mock):
        ...
        with patch.object(Classifier, "_load_logs_for_resource") as log_resource_mock:
            ...
            self._classifier._classify_payload(
                self._mock_payload([self._mock_payload_record()])
            )

            log_resource_mock.assert_called_with(
                self._service_name, self._resource_name
            )
    ...
    def test_classify_payload_bad_record(self):
        ...
        with patch.object(Classifier, "_process_log_schemas"), patch.object(
            Classifier, "_log_bad_records"
        ) as log_mock:
    ...
    def test_run(self, classifiy_mock):
        """Classifier - Run"""
        with patch.object(
            classifier_module.StreamPayload, "load_from_raw_record"
        ) as load_mock:
    ...
    @patch.object(Classifier, "_classify_payload")
    def test_run_artifact_extractor_disabled(
        self, classifiy_mock, artifact_extractor_mock
    ):
    ...
    @patch("streamalert.shared.artifact_extractor.ArtifactExtractor.run")
    @patch.object(Classifier, "_classify_payload")
    def test_run_artifact_extractor_enabled(
        self, classifiy_mock, artifact_extractor_mock
    ):
