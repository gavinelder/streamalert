...
class TestOutputDispatcher:
    ...
    def test_credentials_provider(self, provider_constructor):
        ...
        provider_constructor.assert_called_with(
            "test_service", config=CONFIG, defaults=None, region=REGION
        )
    ...
    def test_log_status_success(self, log_mock):
        ...
        log_mock.assert_called_with(
            "Successfully sent alert to %s:%s", "test_service", self._descriptor
        )
    ...
    def test_log_status_failed(self, log_mock):
        ...
        log_mock.assert_called_with(
            "Failed to send alert to %s:%s", "test_service", self._descriptor
        )
    ...
    @patch.object(
        OutputDispatcher, "_get_exceptions_to_catch", Mock(return_value=(ValueError))
    )
