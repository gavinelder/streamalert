...
class TestPagerDutyOutput:
    ...
    def test_dispatch_success_with_contexts(self, post_mock, log_mock, compose_alert):
        ...
        compose_alert.return_value = {
            "@pagerduty.contexts": [
                {"type": "link", "href": "https://streamalert.io", "text": "Link text"},
                {
                    "type": "image",
...
@patch(
    "streamalert.alert_processor.outputs.pagerduty.PagerDutyIncidentOutput.BACKOFF_MAX",
    0,
)
@patch(
    "streamalert.alert_processor.outputs.pagerduty.PagerDutyIncidentOutput.BACKOFF_TIME",
    0,
)
class TestPagerDutyIncidentOutput:
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_sends_correct_put_incident_request(
        self, get_mock, post_mock, put_mock
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_sends_correct_with_assigned_user(
        self, get_mock, post_mock, put_mock
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_sends_correct_with_invalid_assigned_user(
        self, get_mock, post_mock, put_mock, log_info_mock, log_warn_mock
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_sends_correct_with_urgency(
        self, get_mock, post_mock, put_mock, compose_alert
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_sends_correct_bad_urgency(
        self, get_mock, post_mock, put_mock, compose_alert, log_mock
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_success_good_policy(
        self, get_mock, post_mock, put_mock, log_mock
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_success_with_priority(
        self, get_mock, post_mock, put_mock, log_mock
    ):
        ...
        put_mock.assert_any_call(
            ...
            json={
                "incident": {
                    ...
                    "priority": {"type": "priority_reference", "id": "priority_id"},
                }
            },
            timeout=3.05,
            verify=False,
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_no_dispatch_no_event_response(
        self, get_mock, post_mock, log_mock
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_no_dispatch_no_incident_response(
        self, get_mock, post_mock, put_mock, log_mock
    ):
        ...
        log_mock.assert_any_call(
            "[%s] Failed to update container incident for event", self.SERVICE
        )
        log_mock.assert_called_with(
            "Failed to send alert to %s:%s", self.SERVICE, self.DESCRIPTOR
        )
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_no_dispatch_no_incident_id_in_response(
        self, get_mock, post_mock, put_mock, log_mock
    ):
        ...
        RequestMocker.setup_mock(
            ...
            [
                [
                    re.compile(r"^.*/incidents/[a-zA-Z0-9-_]+$"),
                    200,
                    {"incident": {"not_id": "?"}},
                ],
            ],
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_request_responder_single(
        self, get_mock, post_mock, put_mock, log_mock
    ):
    ...
    @patch("requests.post")
    @patch("requests.get")
    def test_dispatch_request_responder_not_valid(
        self, get_mock, post_mock, put_mock, log_mock, log_error_mock
    ):
...
class TestWorkContext:
    ...
    def test_get_standardized_priority_success(self, get_mock):
        ...
        get_mock.return_value.status_code = 200
        json_check = {
            "priorities": [{"id": "verified_priority_id", "name": priority_name}]
        }
    ...
    def test_get_standardized_priority_not_found(self, get_mock):
        ...
        get_mock.return_value.status_code = 200
        json_check = {
            "priorities": [{"id": "verified_priority_id", "name": "not_priority_name"}]
        }
        get_mock.return_value.json.return_value = json_check
    ...
    def test_get_standardized_priority_invalid(self, get_mock):
        ...
        get_mock.return_value.status_code = 200
        json_check = {
            "not_priorities": [{"id": "verified_priority_id", "name": "priority_name"}]
        }
        get_mock.return_value.json.return_value = json_check
...
class TestPagerDutyRestApiClient:
    ...
    def setup(self, _):
        ...
        self._api_client = PagerDutyRestApiClient(
            "mocked_token", "user@email.com", http
        )
...
class TestJsonHttpProvider:
    ...
    def test_get_sends_correct_arguments(self):
        ...
        self._dispatcher._get_request_retry.assert_called_with(
            "http://airbnb.com", {"q": "zz"}, {"Accept": "application/tofu"}, True
        )
...
class TestWorkContextUnit:
    ...
    def test_unstable_note_fail(self, compose_alert_mock, log_error):
        ...
        log_error.assert_called_with(
            StringThatStartsWith("[test] Failed to add note to incident")
        )
