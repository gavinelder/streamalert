...
class TestPhantomOutput:
    ...
    @patch(
        "streamalert.alert_processor.outputs.output_base.OutputDispatcher._get_request"
    )
    @patch(
        "streamalert.alert_processor.outputs.output_base.OutputDispatcher._post_request"
    )
    def test_dispatch_container_query(self, post_mock, get_mock):
        ...
        assert not PhantomOutput._setup_container(
            "rule_name", rule_description, self.CREDS["url"], headers
        )
