...
from pytest import assert_equal, assert_false, assert_true, assert_set_equal
...
class TestSlackOutput:
    ...
    def test_format_message_single(self):
        ...
        assert (
            loaded_message["text"] == "*StreamAlert Rule Triggered: test_rule_single*"
        )
    ...
    def test_json_to_slack_mrkdwn_dict(self):
        ...
        simple_dict = OrderedDict(
            [("test_key_01", "test_value_01"), ("test_key_02", "test_value_02")]
        )
                    ...
                    OrderedDict(
                        [
                            ...
                            (
                                "nested_nested_01",
                                OrderedDict(
                                    [("nested_nested_key_01", [6161, 1051, 51919])]
                                ),
                            ),
                        ]
                    ),
    ...
    def test_max_attachments(self, log_mock):
        ...
        log_mock.assert_called_with(
            "%s: %d-part message truncated to %d parts", alert_publication, 21, 20
        )
