...
class TestCredentialsEncrypted:
    def setup(self):
        ...
        self._encrypted_payload = encrypt_with_kms(
            self._plaintext_payload, REGION, KMS_ALIAS
        )
...
def test_constructor_loads_from_os_when_not_provided():
    ...
    provider = OutputCredentialsProvider(
        "that_service_name", config=CONFIG, region=REGION
    )
        ...
        props = OrderedDict(
            [
                (
                    "property1",
                    OutputProperty(
                        description="This is a property and not a cred so it will not save"
                    ),
                ),
...
class TestLocalFileDriver:
    ...
    def test_save_and_load_credentials_persists_statically(self):
        ...
        driver2 = LocalFileDriver(
            REGION, "service"
        )  # Create a separate, identical driver
