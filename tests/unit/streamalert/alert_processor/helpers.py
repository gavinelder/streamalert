...
def get_random_alert(key_count, rule_name, omit_rule_desc=False):
    ...
    rule_description = (
        "No rule description provided" if omit_rule_desc else "rule test description"
    )
...
def put_mock_ssm_parameters(
    parameter_name, parameter_value, kms_key_alias, region="us-east-1"
):
