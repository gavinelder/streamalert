...
from pytest import assert_equal, assert_is_instance, assert_is_none
...
class TestAlertProcessor:
    ...
    @patch(
        "streamalert.alert_processor.main.load_config",
    ...
    def test_update_alerts_table_update(self):
        ...
        self.processor._update_table(
            self.alert, {"out1": True, "out2": False, "out3": False}
        )
    ...
    def test_run_invalid_alert(self, mock_logger):
        ...
        assert {} == result
        mock_logger.exception.called_once_with(
            "Invalid alert %s", {"Record": "Nonsense"}
        )
