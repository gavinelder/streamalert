...
from pytest import assert_equal, nottest
...
class TestTestEvent:
    ...
    def basic_config():
        return {
            "logs": {
                "misc_log_type": {"schema": {"key": "string"}},
                "override_log_type": {
                    "schema": {"default": "string", "override": "string"}
                },
            }
        }
    ...
    def test_description(self):
        ...
        assert (
            self._default_event.description == "Integration test event for unit testing"
        )
    ...
    def test_is_valid(self):
        ...
        assert self._default_event.is_valid(self.basic_config()) == True
    ...
    def test_is_valid_missing_key(self):
        ...
        assert self._default_event.is_valid(self.basic_config()) == False
        assert (
            self._default_event.error == "Missing required key(s) in test event: 'log'"
        )
    ...
    def test_is_valid_extra_keys(self, log_mock):
        ...
        log_mock.assert_called_with(
            "Additional unnecessary keys in test event: %s", "'extra_thing'"
        )
